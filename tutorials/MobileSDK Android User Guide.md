...
The Mobile SDK for Android is compatible with Android 4.1.x-9.x and has been tested on the Nexus 7, Nexus 5, Samsung Note 3, Samsung Note 5, Samsung S7, HTC Desire 626, HTC One A9, HTC 10, LG G2, LG G3, LG G5, LG G6, SONY XPERIA Z5, SONY XPERIA XZ, General Mobile GM 5+, Huawei P10.
...
The Mobile SDK performs three error checks. If validation fails, the Mobile SDK fills the `error` parameter with the related `MobileError` object. The three error checks include:
...
```java
public void incomingCallNotificationExample(String notificationMessage) {
  EventService eventService = ServiceProvider.getInstance(getApplicationContext()).getEventService();
  try {
    eventService.receiveNotification(notificationMessage);
